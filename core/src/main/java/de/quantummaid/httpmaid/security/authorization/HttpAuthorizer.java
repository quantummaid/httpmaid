...
public interface HttpAuthorizer extends Authorizer {
    ...
    default boolean isAuthorized(final MetaData metaData) {
        final Optional<Object> authenticationInformation = metaData.getOptional(AUTHENTICATION_INFORMATION);
        final HttpRequest request = httpRequest(metaData);
        return isAuthorized(authenticationInformation, request);
    }

    @SuppressWarnings("OptionalUsedAsFieldOrParameterType")
    boolean isAuthorized(Optional<Object> authenticationInformation, HttpRequest request);
}
