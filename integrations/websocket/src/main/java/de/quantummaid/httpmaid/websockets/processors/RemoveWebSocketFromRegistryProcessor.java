...
public final class RemoveWebSocketFromRegistryProcessor implements Processor {
    ...
    public static Processor removeWebSocketFromRegistryProcessor(final WebSocketRegistry registry) {
        validateNotNull(registry, "registry");
        return new RemoveWebSocketFromRegistryProcessor(registry);
    }

    @Override
    public void apply(final MetaData metaData) {
        final WebSocketId webSocketId = metaData.get(WEBSOCKET_ID);
        registry.unregister(webSocketId);
    }
}
