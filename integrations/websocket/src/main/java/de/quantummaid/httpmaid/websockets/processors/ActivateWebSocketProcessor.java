...
public final class ActivateWebSocketProcessor implements Processor {
    private final WebSocketRegistry registry;

    public static Processor activateWebSocketProcessor(final WebSocketRegistry registry) {
        validateNotNull(registry, "registry");
        return new ActivateWebSocketProcessor(registry);
    }

    @Override
    public void apply(final MetaData metaData) {
        final WebSocketId webSocketId = metaData.get(WEBSOCKET_ID);
        registry.activate(webSocketId);
    }
}
