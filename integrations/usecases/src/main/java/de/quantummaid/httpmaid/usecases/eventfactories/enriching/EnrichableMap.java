...
import static de.quantummaid.httpmaid.usecases.eventfactories.enriching.Enrichable.enrichable;
...
public final class EnrichableMap {
    ...
    public static EnrichableMap enrichableMap(final Map<String, Object> map) {
        final Map<String, Enrichable> enrichableMap = new LinkedHashMap<>();
        map.forEach((key, value) -> {
            final Enrichable enrichable = enrichable(key);
            enrichable.setValue(value);
            enrichableMap.put(key, enrichable);
        });
        return new EnrichableMap(enrichableMap);
    }

    public static EnrichableMap enrichableMap(final Collection<String> topLevelKeys) {
