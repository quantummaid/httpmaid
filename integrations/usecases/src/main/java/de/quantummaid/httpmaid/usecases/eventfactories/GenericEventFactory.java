...
import static de.quantummaid.httpmaid.usecases.eventfactories.enriching.EnrichableMap.enrichableMap;
...
public final class GenericEventFactory implements EventFactory {
    ...
    public static EventFactory genericEventFactory(final Collection<String> topLevelKeys) {
        return new GenericEventFactory(topLevelKeys);
    }

    @SuppressWarnings("unchecked")
    @Override
    public EnrichableMap createEvent(final Object unmarshalledBody) {
        final EnrichableMap enrichableMap = enrichableMap(topLevelKeys);
        if (unmarshalledBody instanceof Map) {
            enrichableMap.overwriteTopLevel((Map<String, ?>) unmarshalledBody);
        }
