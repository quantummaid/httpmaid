...
public final class CloudformationTemplate {
    ...
    public String render() {
        final Map<String, Object> resourcesMap = resources.stream()
                .collect(toMap(x -> {
                    final CloudformationName cloudformationName = x.name();
                    return cloudformationName.asId();
                }, CloudformationResource::render));
        ...
        if (!outputs.isEmpty()) {
            ...
            outputs.forEach(output -> {
                final CloudformationName name = output.name();
                final Object value = output.value();
                outputsMap.put(name.asId(), Map.of("Value", value));
            });
